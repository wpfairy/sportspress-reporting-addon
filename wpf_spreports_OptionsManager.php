...
class Wpf_plugin_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'wpf_plugin'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'wpf_plugin'); ?></h2>
            <table cellspacing="1" cellpadding="2"><tbody>
            <tr><td><?php _e('System', 'wpf_plugin'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'wpf_plugin'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'wpf_plugin');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'wpf_plugin'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'wpf_plugin');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'wpf_plugin'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'wpf_plugin') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'wpf_plugin');
            case 'false':
                return __('false', 'wpf_plugin');

            case 'Administrator':
                return __('Administrator', 'wpf_plugin');
            case 'Editor':
                return __('Editor', 'wpf_plugin');
            case 'Author':
                return __('Author', 'wpf_plugin');
            case 'Contributor':
                return __('Contributor', 'wpf_plugin');
            case 'Subscriber':
                return __('Subscriber', 'wpf_plugin');
            case 'Anyone':
                return __('Anyone', 'wpf_plugin');
        }
        return $optionValue;
    }
